...
public class StreamTaskImpl {
    ...
    public double calculateStandardDeviation(Blackhole blackhole) {
        ...
        double standardDeviation = Math.sqrt(collection.stream()
    ...
    public List<Long> multiplyCollectionByTwo(Blackhole blackhole) {
        List<Long> multipliedValues = collection.stream()
                                                .map(num -> num * 2)
                                                .collect(Collectors.toList());
    ...
    public List<Long> filterCollection(Blackhole blackhole) {
        List<Long> filteredValues = collection.stream()
